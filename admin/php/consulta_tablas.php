...
function mostrar_diplomasCursos()
{
    ...
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        if (is_null($row['catalogo_productos_sku']) === false) {
            $skuProd = $row['catalogo_productos_sku'];
        }
        if (is_null($row['catalogo_productos_nombre']) === false) {
            $nameProd = $row['catalogo_productos_nombre'];
        }
        if (is_null($row['categorias_programas_nombre']) === false) {
            $tipoProd = $row['categorias_programas_nombre'];
        }
        if (is_null($row['catalogo_productos_fechainicio']) === false) {
            $initialDateProd = date('d/m/Y', strtotime($row['catalogo_productos_fechainicio']));
        }
        if (is_null($row['catalogo_productos_fechafin']) === false) {
            $EndDateProd = date('d/m/Y', strtotime($row['catalogo_productos_fechafin']));
        }
        if (is_null($row['catalogo_productos_preciomx']) === false) {
            $preciomxProd = number_format($row['catalogo_productos_preciomx'], 2);
        }
        if (is_null($row['modalidad_nombre']) === false) {
            $modality = $row['modalidad_nombre'];
        }
        if (is_null($row['catalogo_productos_duracion']) === false) {
            $duration = $row['catalogo_productos_duracion'];
        }
        if (is_null($row['catalogo_productos_descripcioncorta']) === false) {
            $descCorta = $row['catalogo_productos_descripcioncorta'];
        }
        if (is_null($row['catalogo_productos_incluye']) === false) {
            $infoObjetivo = $row['catalogo_productos_incluye'];
        }
