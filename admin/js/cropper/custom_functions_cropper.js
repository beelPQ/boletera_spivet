...
const inputCropper = (inputFile,idPreview,widthPreview,heightPreview,widthCrop,heightCrop,typeCrop,minWidthCrop,minHeightCrop,maxWidthCrop,maxHeightCrop,e) =>{
    ...
    let done = function (url){
        ...
        /**
         * *Validacion exclusiva para el modal de credencial
         */
        // e.target.dataset.imagename
        if( e.target.dataset.imagetype ){
            const imageType = e.target.dataset.imagetype;
            btnSaveCrop.setAttribute("onclick", "saveCrop('"+idPreview+"',"+widthCrop+","+heightCrop+","+widthPreview+","+heightPreview+",event,'"+imageType+"')");
        }else{
...
const saveCrop = (idPreviewCrop,widthCrop,heightCrop,widthPreview,heightPreview,e, call = "") =>{
    ...
    /**
     **Funcion exclusiva para el recorte de imagen de credencial
     **/
    if( call == "imageLogoSec1" ){
        if( document.querySelector(`#imgLogoSelected`) ){
            document.querySelector(`#imgLogoSelected`).src = canvas.toDataURL();  
        }
    }
    if( call == "imageBgSec2" ){
        if( document.querySelector(`#imgBgSelectedSec2`) ){
            document.querySelector(`#imgBgSelectedSec2`).src = canvas.toDataURL();  
        }
    }
...
const cleanPreview = (btnClean,idPreview,e) =>{
    ...
    if(e.target.dataset.deleteimagetype){
        const imageDelete = e.target.dataset.deleteimagetype;
        if( imageDelete == "imageLogoSec1" ){
            document.querySelector(`#imgLogoSelected`).src= "";
        }
        
        if( imageDelete == "imageBgSec2" ){
