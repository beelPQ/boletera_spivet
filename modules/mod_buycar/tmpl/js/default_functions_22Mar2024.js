    ...
    .then((response) => {
        ...
        if( response.status_code==200 ) {
            ...
            if(localStorage.getItem(`cart_products`) && JSON.parse(localStorage.getItem(`cart_products`)).length > 0){
                ...
                //console.log(`si existe la sesion `);
            }
            ...
            arrayProducts.forEach(function(product) {
                ...
                if(product.catalogo_productos_publicado==1){
                    ...
                    //console.log(product.catalogo_productos_nombre)
                    //console.log(product.pricemxn_discount)
...
const deleteProduct = (btnDelPro) =>{
    ...
    */
    ...
    setTimeout(() => {
        ...
        let condescuento = document.querySelector('#descuento_aplicado').value;
        if (condescuento != "" ) {
            //console.log('osv_calculate_cupon()')
            calculate_cupon();
        }else{
            //console.log('osv_calculate_total()')
            calculate_total();
        }
    }, 200);
...
const getOptionsPayment = () => {
    //console.log('osv____________________FuncionConsultaFormas')
    let formData = new FormData();
    ...
    .then((result) => {
        if(result.status){
            ...
            //console.log('osv___LLenandoFormas1')
            //console.log(options)
            document.querySelector(`#optionsPayment`).innerHTML = options;
            //console.log('osv___LLenandoFormasw')
...
const validateStep3 = () =>{
    ...
    if(selectPaymentMethod.value !== "no_selected"){
        if(checkInvoiceNo.checked){
            console.log("ya no hay que validar nada completado...");
            ...
            if(inputFile.value!== ""){
                ...
                console.error("el input file esta vacio");    
...
const validateStepsComplete = () => {
    ...
    console.log(`erroes paso1: ${finalStep1}     errores paso2: ${finalStep2}   errores paso3: ${finalStep3}`);
    
    if(finalStep1 === 0 && finalStep2 === 0  && finalStep3 === 0 ){
        //console.log('osv_habilitando pay')
